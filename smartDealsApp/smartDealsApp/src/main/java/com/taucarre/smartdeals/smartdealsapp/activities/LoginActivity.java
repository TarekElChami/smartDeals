...
public class LoginActivity extends Activity implements LoaderCallbacks<Cursor>{
     ...
     * the user.
     */
    public class UserLoginTask extends AsyncTask<Void, Void, HashMap<String,Boolean>> {
        ...
        protected HashMap<String,Boolean> doInBackground(Void... params) {
            ...
            User user = usersDataDao.getUserByMail(mEmail);
            usersDataDao.close();
            if(user != null){
                map.put("mail",true);
                if(mPassword.equals(user.getPassword())){
                    smartDealsApplication.setUserAthentifie(true);
        ...
        @Override
        protected void onPostExecute(final HashMap<String,Boolean> success) {
            mAuthTask = null;
            showProgress(false);

            if (success.get("password")) {
                finish();
            } else {
                if (success.get("mail")) {
                    ...
                    intent.putExtra("user", "Utilisateur non enregistré");
                    intent.setAction("SMARTDEALS_CONFIGURER_PROFIL_ACTIVITE");
